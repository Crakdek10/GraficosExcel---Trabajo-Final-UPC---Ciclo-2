...
def Menu():
    print("\033[1;36m"+'Lista de reportes'+"\033[0;m")
    print("""
...
def R1_Porc_Hombres_Mujeres_VIH_Castilla_Leon():
    ...
    print("\033[1;36m"+"Numero de población en Castilla y León\t Sexo\t Cantidad de personas con VIH\t Porcentaje "+"\033[0;m")
    print('-------------------------------------------------------------------------------------------')
    print('\t\t\t\t\t  H','\t\t   ', df_hombres['Sexo'].count(), '\t\t ' ,porch,'%')          
    print('\t   2408 millones')
    print('\t\t\t\t\t  M','\t\t   ', df_mujeres['Sexo'].count(), '\t\t ' ,porcm,'%')  
...
def R2_Provincia_Mayor_Casos_VIH_1982_a_2022():
    ...
    ListProvincias = df['provincia'].drop_duplicates()            #PRIMER CAMBIO
    provincia_contador = {}
    ...
    print("\033[1;36m"+"Provincia\tCantidad de casos"+"\033[0;m")    
    print('---------------------------------')
    for provincia, num in provincia_contador.items():
     print('{:<15}\t\t{}'.format(provincia, num))
    
    total = sum(provincia_contador.values())
    print('Total\t\t       ', total)
    print("\033[1;36m"+"\nProvincia con más casos de VIH:"+"\033[0;m", max(provincia_contador, key=provincia_contador.get))
...
def R3_Tendencia_Casos_VIH_2000_a_2022():
    ...
    print("\033[1;36m"+"Año\tCasos de VIH"+"\033[0;m")
    print('----------------------------')
    for año, count in año_contador.items():
     print(año,'\t',count)
...
def R4_Promedio_Edades_Según_Década():
    ...
    ndatos = df.groupby((df['Año de diagnóstico'] // 10) * 10)['Edad años'].count()
    
    colores = ["#F2D7D5","#E6B0AA","#D98880","#CD6155","#C0392B"]
    lista = ["1980-1989","1990-1999","2000-2009","2010-2019","2020-2022"]
    
    print("\033[1;36m"+'Década\t\tPromedio de edad\tN° Datos'+"\033[0;m")
    print('-------------------------------------------------')
    for num in range(0,5):
     print(lista[num],'\t\t', promedios.values[num],'\t\t',ndatos.values[num])
...
def R5_Frecuencia_Vías_De_Transmisión_VIH_1982_a_2022():
 ...
 Grupos=df['Grupo de riesgo '].drop_duplicates().tolist()
 ...
 print("\033[1;36m"+'Código\t\tGrupo de Riesgo\t\t\t\t\tN° Frecuencia'+"\033[0;m")
 print('-----------------------------------------------------------------------------')
 for num in range(0,8):
  print('{:<10}{:<60}{}'.format(10+10*num, Grupos[num], Frecuencia_grupos.values[num]))

 plt.figure(figsize=(14,7))
 plt.bar(lista, Frecuencia_grupos.values,color = colores)
      
 plt.title("Frecuencia de las principales \nvías de transmisión de VIH entre 1982 y 2022",
           fontsize=20)
...
###############################################################################
df = pd.read_csv('Excel_VIH.csv', delimiter = ';')
os.system('cls')
Mostrar_Bienvenida()
